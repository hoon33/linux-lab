...
cp src_folder test_folder # -r 옵션 차이 확인 : test 디렉토리를 복사하지 못해서
...
mv newname.txt /root/workspace/logs/ # 에러 확인
...
```bash
grep "error" system.txt # 안나옴
grep "ERROR" system.log # 대소문자를 구별한다?
grep -i "error" system.log # -i : 대소문자 무시
```
...
```
log 폴더를 만들고   # mkdir log
system.txt 파일에서  
ERROR로 시작하는 줄만 모아  
log/error_summary.txt 파일로 저장하세요.

a.txt, b.txt 파일을 만들고 병합하세요 a.txt는 hello   b.txt는 world
cat a.txt b.txt > all.txt
cat all.txt
